...
for i in range(numberOfPackets):
    ...
    # 1465 bytes payload
    message += "A" * 1465 # Fill the packet with 'A' characters to reach the desired payload size why? Because the total packet size (header + payload) should be 1500 bytes, and the header is typically around 35 bytes for UDP, so we need to fill the payload with 1465 bytes of data to reach the total size of 1500 bytes.
